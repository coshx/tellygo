  ...
  module IO
    class AMFSerializer
      ...
      def write(value)
        ...
        elsif defined?(ActiveRecord) && value.is_a?(ActiveRecord::Base) # Aryk: this way, we can bypass the next four checks most of the time
      ...
      def write_amf3(value)
        ...
        elsif defined?(ActiveRecord) && value.is_a?(ActiveRecord::Base) # Aryk: this way, we can bypass the "['REXML::Document', 'BeautifulSoup'].include?(value.class.to_s) " operation
          write_amf3_object(value)
